...
2. **Model routing.** Most agent steps are easy. Use a small fast model (~3–8B)
   for routine steps and only escalate to a big local model — or **the cloud model** —
...
| **Entry** | **16 GB VRAM** | 7–14B quantized | Workable; lean harder on the cloud model escalation for hard tasks |
...
## Local-first with the cloud model escalation

Stay local by default; escalate to the **the cloud model API** only when it's worth it:
...
- **Result:** local = the private, fast default; the cloud model = on-demand horsepower.
...
step ─▶ small local model ──(easy)──▶ act
          ...
          (hard / low confidence / high stakes)
             ▼
     big local model ──(still stuck / needs huge context)──▶ the cloud model API
```
...
| Hard reasoning (big local / the cloud model) | seconds — used sparingly |
